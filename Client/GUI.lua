...
local function DrawButton(b)
	...
	if b.Shadow then
		DrawSprite("mg_clothing", "circle", b.x, b.y, b.Size.Circle.x/0.80, b.Size.Circle.y/0.80, Rot, b.Colour.r, b.Colour.g, b.Colour.b, b.Alpha)
	end
	DrawSprite("mg_clothing", b.Sprite, b.x, b.y, b.Size.Sprite.x/0.68, b.Size.Sprite.y/0.68, b.Rotation, 255, 255, 255, b.Alpha)
...
local function DrawGUI()
	...
	for k,v in pairs(Buttons) do
		...
		if LastEquipped[FirstUpper(v.Sprite)] then
			Alpha = 180 Colour = {r=2,g=110,b=245,a=37}
		else 
			Alpha = 255 Colour = {r=2,g=110,b=245,a=37}
		end
		DrawSprite("mg_wheel", k.."", x, y, 0.4285, 0.7714, 0.0, Colour.r, Colour.g, Colour.b, Colour.a)
	...
	if Cooldown then Text(x, y+0.05, 0.28, Lang("Waitasec"), false, false, true) end 		
	...
	if InfoButton then 			
		...
		for k,v in pairs(LastEquipped) do log(k.." : "..json.encode(v)) end		
	end
end

local TextureDicts = {"mg_clothing", "mg_wheel"}
Citizen.CreateThread(function()
